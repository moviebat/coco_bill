...
def check_image_label(image_dir, label_file):
    ...
    try:
        for line in file_object:
            ...
            if (not my_file.exists()) and (not 'frame_' in billiards_picture_name) :# 指定的文件存在
...
def  check_filename(root_path):
    ...
    result = [0,0,0,0,0]
    success_count, failure_count, directory_count, directory_succ_count, directory_fail_count  = 0
    new_label_file = ''
...
def main():
    ...
    logger.info('共完成图片拷贝%d张，失败%d个' % (result[3], result[4]))
